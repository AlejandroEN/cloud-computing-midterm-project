...
export class PostsService {
	...
	async create(
		...
		const uploadedUrls = await Promise.all(
			images.map((file, idx) =>
				this.s3Service.uploadFile(file, `${profileId}-${idx}`)
			)
		);
	...
	async update(
		...
		if (images && images.length > 0) {
			...
			await Promise.all(
				post.images.map((url) => this.s3Service.deleteFile(url))
			);
	...
	async findAllByOwner(profileId: string) {
		...
		return this.postsModel.find({ seller_id: profileId }).lean().exec();
	}
	...
	async findWithFilters(query: PostsQueryDto, ownerId: string) {
		...
		if (nameContains)
			filter.title = { $regex: nameContains, $options: "i" };
	...
	async ratePost(id: string, rating: number, raterId: string) {
		...
		if (post.seller_id === raterId) {
			throw new ForbiddenException(
				"The creator of the post cannot rate it"
			);
		}
