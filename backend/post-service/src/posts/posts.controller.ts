...
export class PostsController {
	...
	@ApiOperation({
		summary: "Obtiene los posts públicos de un perfil por su ID",
	})
	@Get("profile/:profileId")
	async findPublicByProfile(@Param("profileId") profileId: string) {
		return this.postsService.findPublicByProfile(profileId);
	}

	@ApiOperation({ summary: "Obtiene todos los posts con filtros" })
	@Get()
	async findAll(@Request() req, @Query() query: PostsQueryDto) {
	...
	@Patch("undelete/:id")
	async undelete(@Param("id") id: string, @Request() req) {
		...
		if (post.seller_id !== req.user.profileId) {
			throw new ForbiddenException(
				"You do not have permission to undelete this post."
			);
		}
	...
	async unarchive(@Param("id") id: string, @Request() req) {
		...
		if (post.seller_id !== req.user.profileId) {
			throw new ForbiddenException(
				"You do not have permission to unarchive this post."
			);
		}
	...
	@Patch("anonymous/:id")
	async makeAnonymous(@Param("id") id: string, @Request() req) {
		const post = await this.postsService.findOne(id);
		if (!post) throw new NotFoundException("Post not found");

		if (post.seller_id !== req.user.profileId) {
			throw new ForbiddenException(
				"You do not have permission to anonymous this post."
			);
		}
	...
	async makePublic(@Param("id") id: string, @Request() req) {
		const post = await this.postsService.findOne(id);
		if (!post) throw new NotFoundException("Post not found");

		if (post.seller_id !== req.user.profileId) {
			throw new ForbiddenException(
				"You do not have permission to unanonymous this post."
			);
		}
